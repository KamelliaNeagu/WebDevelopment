...
const vremeaBtn = document.getElementById('vremeaBtn');
const vremeaDiv = document.getElementById('vremeaDiv')
const prognozaBtn = document.getElementById('prognozaBtn');
const prognozaDiv = document.getElementById('prognozaDiv');
var forecast = document.getElementById('viewForecast');
var weather = document.getElementById('viewWeather');
...
function showWeather() {
		...
		if(cityInput.value =='') {
			alert("Va rugam introduceti locatia!");
			return false;
		} else {
...
function createPrognozaHoursOutput(day, icon, hour, temp, description) {
	let output = `
		<div class="prognozaHours">
			<h4 class="date">Ziua: ${day}</h4>
			<img src="${URL_WEATHER_ICON_PREFIX}${icon}.png">
			<h5 class="hour">Ora: ${hour}</h5>
			<h5 class="temp">Temperatura: ${temp}</h5>
			<h5 class="desc">Descriere: ${description}</h5>
		</div>		
		`;
	return output;
}
